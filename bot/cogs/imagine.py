...
class UpscaleView(discord.ui.View):
    """画像生成結果に添付される、拡大ボタン付きの View。"""

    def __init__(self, bot: Athena, image_path: Path, prompt: str):
    ...
    async def upscale_2x(
        ...
        if self.bot.image_service is None or self.bot.queue_service is None:
            await interaction.response.send_message(
                embed=error_embed("アップスケールサービスが利用できません。"),
                ephemeral=True,
            )
            return
        ...
        try:
            ...
            if not self.image_path.exists():
                await interaction.followup.send(
                    embed=error_embed("元の画像ファイルが見つかりませんでした（キャッシュ期限切れの可能性があります）。"),
                    ephemeral=True,
                )
                return
            ...
            embed.set_footer(text=f"{interaction.user.display_name} のリクエスト")
            ...
            logger.exception("Upscale from button failed")
            await interaction.followup.send(
                embed=error_embed(f"拡大に失敗しました: {e}"),
                ephemeral=True,
            )
        finally:
...
class ImagineCog(commands.Cog, name="Imagine"):
    ...
    async def imagine(
        ...
        try:
            ...
            # ボタン付きの View を作成
            view = UpscaleView(self.bot, result.path, prompt)
